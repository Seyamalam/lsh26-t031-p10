...
export function ComparisonView() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Same consumption and calendar-month slab counter.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-[10px]">
            {comparison.days} daily readings
          </Badge>
          <Button
            variant="outline"
            size="sm"
            onClick={() =>
              downloadTextFile(
                ...
                comparisonCsv({
                  ...
                  months: activeCase.comparison.months,
                  ...comparison,
                })
              )
            }
      ...
      <Alert
        id="habit-invariant"
        className={`${comparison.invariant ? "border-teal-500/30 bg-teal-500/5" : ""} scroll-mt-20`}
        variant={comparison.invariant ? "default" : "destructive"}
      >
        ...
        <AlertTitle>
          {comparison.invariant
            ? "Invariant passed: energy and VAT match exactly"
            : "Invariant failed"}
        </AlertTitle>
        <AlertDescription>
          Recharge timing cannot change energy slab cost. Any difference below
          comes only from monthly first-recharge fixed charges.
        </AlertDescription>
      </Alert>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {activeCase.comparison.months.map(prettyMonth).join(" · ")}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ComparisonCostChart
              low={comparison.lowBalance}
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="flex items-center gap-2 text-xl">
              <Equal className="size-5 text-teal-600" /> {result}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <PolicyRow
              label="Energy cost"
              low={comparison.lowBalance.energyPoisha}
              monthly={comparison.monthly.energyPoisha}
            />
            <PolicyRow
              label="VAT"
              low={comparison.lowBalance.vatPoisha}
              monthly={comparison.monthly.vatPoisha}
            />
            <PolicyRow
              label="Fixed charges"
              low={comparison.lowBalance.fixedPoisha}
              monthly={comparison.monthly.fixedPoisha}
            />
            <Separator />
            <PolicyRow
              label="Consumed cost"
              low={comparison.lowBalance.costPoisha}
              monthly={comparison.monthly.costPoisha}
              strong
            />
          </CardContent>
        </Card>
      ...
      <p className="text-xs text-muted-foreground">
        Deposits are cash added, not cost. Cost is energy + VAT + applicable
...
function PolicyRow({
  ...
  return (
    <div
      className={`grid grid-cols-[1fr_auto_auto] items-center gap-4 ${strong ? "font-semibold" : ""}`}
    >
      <span>{label}</span>
