...
export function AdvisorView() {
  ...
  const [selection, setSelection] = useState({
    ...
    date: activeCase.target_date,
  })
  const targetDate =
    selection.caseId === activeCase.case_id
      ? selection.date
      ...
      <div className="flex items-center gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Run-out date and exact deposit requirement.
          </p>
        </div>
        ...
        <Card id="run-out-answer" className="scroll-mt-20">
          <CardHeader>
            <CardDescription className="flex items-center gap-2">
              <CalendarClock className="size-4" /> Projected run-out
            </CardDescription>
            <CardTitle className="text-2xl">
              {runOut ? prettyDate(runOut.date) : "Beyond 10 years"}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              <Stat
                ...
                value={runOut ? String(runOut.days) : "3,650+"}
              />
              <Stat
                label="Usual use"
                value={`${activeCase.usual_daily_units} kWh/day`}
              />
              <Stat
              ...
              <Stat
                label="Month units"
                value={`${last.monthlyUnitsAfter.toFixed(2)} kWh`}
              />
            </div>
            <p className="text-xs leading-5 text-muted-foreground">
              Forecast starts after {activeCase.today}. Slab units reset on the
          ...
          <CardHeader className="border-b">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Calculated in integer poisha through the selected date.
                </CardDescription>
              </div>
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  value={targetDate}
                  onChange={(event) =>
                    event.target.value > activeCase.today &&
                    setSelection({
                      caseId: activeCase.case_id,
          ...
          <CardContent className="grid gap-6 pt-6 md:grid-cols-[0.8fr_1.2fr]">
            <div className="rounded-lg border bg-muted/35 p-5">
              ...
              <p className="mt-2 font-mono text-3xl font-semibold text-teal-700 tabular-nums dark:text-teal-300">
                {formatBdt(advice.rechargeNeededPoisha)}
              </p>
              <p className="mt-3 text-sm text-muted-foreground">
                {advice.projectedDays} days · {advice.projectedUnits.toFixed(2)}{" "}
                projected kWh
              </p>
              <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground">
                <Gauge aria-hidden="true" className="size-4" /> Through{" "}
                {prettyDate(targetDate)}
              </div>
            </div>
            <div className="space-y-2 text-sm">
              <Breakdown
                label="Energy at first-slab rate"
                value={advice.baselineEnergyPoisha}
              />
              <Breakdown
                label="Higher-slab increment"
                value={advice.higherSlabPoisha}
              />
              <Breakdown
                label="Energy subtotal"
                value={advice.energyPoisha}
                strong
              />
              <Breakdown label="VAT on energy" value={advice.vatPoisha} />
              <Breakdown
                label="Applicable fixed charges"
                value={advice.fixedPoisha}
              />
              <Separator className="my-3" />
              <Breakdown
                label="Current balance credit"
                value={-advice.balanceCreditPoisha}
                muted
              />
              <Breakdown
                label="Recharge required"
                value={advice.rechargeNeededPoisha}
                strong
              />
            </div>
          </CardContent>
...
function Breakdown({
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 ${muted ? "text-muted-foreground" : ""} ${strong ? "font-semibold" : ""}`}
    >
      <span>{label}</span>
