...
import { ArrowUpRight, CalendarClock, CheckCircle2, FileSearch, ReceiptText, Sun, Zap } from "lucide-react"
...
export function JudgeShortcuts() {
  const { activeCase, ledger, last, monthly, runOut, comparison, alreadyRechargedThisMonth } = useFixture()
  const lateRecharge = activeCase.recharges
    ...
    .reduce<(typeof activeCase.recharges)[number] | null>((largest, item) => {
      if (!largest || parseBdt(item.amount_bdt) > parseBdt(largest.amount_bdt)) return item
      return largest
    }, null)
  ...
  const shortcuts = [
    { href: "/dashboard#history-checks", label: "Light month", value: `${prettyMonth(monthly.light.month)} · ${monthly.light.units} units`, icon: Sun },
    { href: "/dashboard#history-checks", label: "Heavy summer month", value: `${prettyMonth(monthly.heavy.month)} · ${monthly.heavy.units} units`, icon: Zap },
    {
      href: "/dashboard#history-checks",
      label: "Late large recharge",
      value: lateRecharge ? `${prettyDate(lateRecharge.date)} · ${formatBdt(parseBdt(lateRecharge.amount_bdt))}` : "No late recharge",
      icon: CalendarClock,
    },
    {
      href: "/ledger#fixed-charge-evidence",
      label: "First recharge charges",
      value: firstFixed ? `${prettyDate(firstFixed.date)} · ${formatBdt(firstFixed.fixedChargesPoisha)}` : "No fixed charge",
      icon: ReceiptText,
    },
    { href: "/advisor#run-out-answer", label: "Run-out answer", value: runOut ? prettyDate(runOut.date) : "Beyond forecast", icon: CalendarClock },
    {
    ...
    },
    {
      ...
      value: comparison.invariant ? `${formatBdt(comparison.lowBalance.energyPoisha)} + ${formatBdt(comparison.lowBalance.vatPoisha)}` : "Invariant failed",
      icon: CheckCircle2,
    },
  ]
  ...
  return (
    <Card className="border-primary/25 bg-primary/[0.025]">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Open the exact evidence for each tariff requirement.</CardDescription>
      </CardHeader>
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-center gap-1 text-xs font-medium">
                {item.label}<ArrowUpRight aria-hidden="true" className="size-3 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
              </span>
              <span className="mt-1 block text-xs leading-4 text-muted-foreground">{item.value}</span>
            </span>
          </Link>
        ))}
      </CardContent>
