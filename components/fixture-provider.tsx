...
export function FixtureProvider({ children }: { children: React.ReactNode }) {
  ...
  const computed = useMemo(() => {
    const ledger = runDailyLedger(parseBdt(activeCase.opening_balance_bdt), activeCase.days, activeCase.recharges)
    ...
    return {
      ...
      alreadyRechargedThisMonth: activeCase.recharges.some((recharge) => recharge.date.startsWith(activeCase.today.slice(0, 7))),
    }
  }, [activeCase])

  const selectCase = (nextCaseId: string | null) => {
    if (nextCaseId && fixture.cases.some((item) => item.case_id === nextCaseId)) {
...
export function useFixture() {
  ...
  if (!context) throw new Error("useFixture must be used inside FixtureProvider.")
  return context
}
