...
describe("all published P10 cases", () => {
  ...
  it.each(document.cases.map((item) => [item.case_id, item] as const))(
    ...
    (_id, item) => {
      const ledger = runDailyLedger(parseBdt(item.opening_balance_bdt), item.days, item.recharges);
      expect(ledger).toHaveLength(item.days.length);
      expect(ledger.every((row) => row.energyCostPoisha >= 0 && row.vatPoisha >= 0)).toBe(true);
      const last = ledger.at(-1)!;
      const comparison = compareHabits(item.days, item.comparison);
      expect(comparison.invariant).toBe(true);
      expect(comparison.lowBalance.energyPoisha).toBe(comparison.monthly.energyPoisha);
      expect(comparison.lowBalance.vatPoisha).toBe(comparison.monthly.vatPoisha);
      expect(comparison.differencePoisha).toBe(Math.abs(comparison.lowBalance.fixedPoisha - comparison.monthly.fixedPoisha));
      expect(forecastRunOut({ date: item.today, balancePoisha: last.closingBalancePoisha, monthlyUnits: last.monthlyUnitsAfter }, item.usual_daily_units)).not.toBeNull();
      const advice = calculateRechargeNeed(
        { date: item.today, balancePoisha: last.closingBalancePoisha, monthlyUnits: last.monthlyUnitsAfter },
        item.target_date,
        item.usual_daily_units,
        item.recharges.some((recharge) => recharge.date.slice(0, 7) === item.today.slice(0, 7)),
      );
      expect(advice.baselineEnergyPoisha + advice.higherSlabPoisha).toBe(advice.energyPoisha);
